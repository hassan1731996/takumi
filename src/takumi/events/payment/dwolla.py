...
class DwollaPaymentCreate(Event):
    start_state = None
...
class DwollaPaymentRequested(Event):
...
class DwollaPaymentRequestFailed(Event):
    start_state = (None, STATES.PENDING)
...
class DwollaPaymentFailed(Event):
...
class DwollaPaymentSuccessful(Event):
